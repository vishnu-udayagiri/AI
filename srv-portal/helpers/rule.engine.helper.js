    ...
    try {
        ...
        if (((oldStateCode !== newStateCode) || ((oldStateCode === newStateCode) && isSez))&& (newStateCode != 'TE' && newStateCode != 'UN')) {
            ...
            if (isNewStateUnionTerritory) {
                ...
                const newCollectedTaxcode = (await getTaxCodeDetails(db, newCollectedTaxRate))?.data?.TAXCODE
            ...
            if(collectedIGST>0 && collectedIGSTRate > 0){
                ...
                const _newCollectedTaxcode = await getTaxCodeDetails(db, newCollectedTaxRate)
        ...
        if((oldStateCode === newStateCode) && !isSez){
            ...
            if(collectedCGSTRate>0){
                const newCollectedTaxcode = (await getTaxCodeDetails(db, collectedCGSTRate))?.data?.TAXCODE
            ...
            //     const newCollectedTaxcode = (await getTaxCodeDetails(db, collectedIGSTRate))?.data?.TAXCODE
