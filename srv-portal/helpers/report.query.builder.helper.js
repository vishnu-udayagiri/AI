...
exports.buildReportQuery = (baseQuery,tableName, input, pageNumber,pageSize,countOnly,invoiceNumberOnly=false) => {
    let query = '';
    let conditions = [];
    let OR_Conditions = [];
    ...
    WITH NumberedInvoices AS (
        ${joinedQuery.replace('SELECT', `SELECT ROW_NUMBER() OVER(ORDER BY ${tableName}.${row_order}) as rn,`)}
    ) 
    SELECT * 
