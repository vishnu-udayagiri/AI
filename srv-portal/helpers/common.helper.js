...
exports.isValidValueExcludingArray = (value) => {
  if (value === "" || value === null || value === undefined) return false;
  return true;
}
...
exports.checkElementsInArray=(inputArray, comparingArray) =>{

  try {
    return inputArray.filter(element => comparingArray.includes(element));
  } catch (error) {
    return []
  }
  
  // if (comparingArray.length === 0 && inputArray.length > 0) {
  //     return "removeInput";
  // }

  // return (inputArray.every(element => comparingArray.includes(element))).toString();
}
...
exports.prioritizeAndSort = (arr, pattern) => {
  ...
  arr.sort((a, b) => {
      ...
      const aMatches = regex.test(aStr);
      const bMatches = regex.test(bStr);
