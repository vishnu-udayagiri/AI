...
exports.getEmbassys = async (db) => {
  try {
    ...
    const query = `SELECT DESCR as	DESCR, CODE as COUNTRY_CODE  FROM SAP_COMMON_COUNTRIES WHERE CODE <> 'IN';
    `;
...
exports.getCountryCodesIATA = async (db) => {
  try {
    ...
    let data = [];
    const query = `SELECT DESCR as	COUNTRYNAME, CODE as COUNTRY_CODE  FROM SAP_COMMON_COUNTRIES WHERE CODE <> 'IN'`;
    data = (await db.exec(query))
