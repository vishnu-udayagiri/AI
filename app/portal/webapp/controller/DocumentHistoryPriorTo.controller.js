sap.ui.define(
    ...
    function (Controller,
        ...
        return Controller.extend("airindiagst.controller.DocumentHistoryPriorTo", {
            ...
            onInit: async function () {
                ...
                this.oRouter.getRoute("DocumentHistoryPriorTo").attachPatternMatched(this._routeMatched, this);
                ...
                var oDateRangeSelection = this.byId("fbdat-DateofIssue");
                var oDateRangeSelection1 = this.byId("fbdat-invoiceDate");
                var oMinDate = new Date(2023, 3, 1);
                var oMaxDate = new Date(2024, 2, 31);

                oDateRangeSelection.setMinDate(oMinDate);
                oDateRangeSelection.setMaxDate(oMaxDate);
                oDateRangeSelection1.setMinDate(oMinDate);
                oDateRangeSelection1.setMaxDate(oMaxDate);
            ...
            _routeMatched: function (oEvent) {
                ...
                this.getOwnerComponent().getModel("shellModel").setProperty("/title", "Documents from April 2023 to March 2024");
            ...
            Fetch_GSTDetails: function () {
                ...
                filterval.issuanceFrom = "2023/04/01";
                filterval.issuanceTo = "2024/03/31";
              
                this.loopfilters = filterval;

                this.SendRequest(this, "/portal-api/portal/v1/get-exhaustive-report", "POST", {}, JSON.stringify(filterval), (_self, data, message) => {
                    if (data) {
                        _self.GSTDetails = data;
                        console.log(data)

                        _self.oModel = new sap.ui.model.json.JSONModel([]);
                        _self.oModel.setData(_self.GSTDetails.documentHistory);
                        var oModelData = new sap.ui.model.json.JSONModel();
                        _self.filterJson = {

                        };
                        _self.filterJson = _self.GSTDetails.filters;
                        _self.defaultGSTN_num = _self.GSTDetails.filters.gstin;
                        ...
                        _self.uniqueGSTDetails.results = _self.GSTDetails.filters.gstin;
                        _self.oModel = new sap.ui.model.json.JSONModel([]);
                        ...
                        //.byId("fbmc-TIMELINE").setSelectedKey(_self.defaultperiod);
            ...
            handleValueHelppassengerGstin: function (oEvent) {
                ...
                if (!this.passengergstinDialog) {
                    this.passengergstinDialog = sap.ui.xmlfragment("airindiagst.view.Fragment.PriorTopassengerGstin", this);
            ...
            handleValueHelpSearchpassengergstin: function (oEvent) {
                ...
                if (invnoSH != "") {
                    this.SendRequest(this, "/portal-api/portal/v1/get-filter-data", "POST", {}, JSON.stringify({ "gstin": invnoSH, "apiType": "DocumentHistoryPriorTo" }), (_self, data, message) => {

                        var oModelData = new sap.ui.model.json.JSONModel();
                        if ((data != null)) {
                            _self.filterJson;
                            _self.filterJsonUpdated = data.filters;
                            _self.filterJson.gstin = _self.filterJson.gstin.concat(data.filters.gstin.filter(function (item) {
                                return _self.filterJson.gstin.indexOf(item) === -1; // Check for uniqueness
                            }));
                            oModelData.setData(_self.filterJson);
                            _self.getView().setModel(oModelData, "FilterDatamodel");
            ...
            onSearch: function (oEvent) {
                if (!this.getView().byId("filterbar").isDialogOpen()) {
                    ...
                    // }
                    if (!oEvent) {
                        this.filtered_GSTDetails.documentHistory = this.GSTDetails.documentHistory.filter(function (item) {
                        ...
                        if (_self.GSTDetails.documentHistory.length > 0) {
                            _self.getView().byId("morelink").setVisible(true);
                            _self.getView().byId("title").setText("Documents(1 - " + _self.GSTDetails.documentHistory.length + " of " + _self.GSTDetails.totalcount + ")");
                            //  _self.getView().byId("morelink").setText("(1 - " + _self.GSTDetails.invoices.length + " of " + _self.GSTDetails.totalInvoices + ") More..");

                        } else {
                            _self.getView().byId("title").setText("Documents(" + _self.GSTDetails.documentHistory.length + ")");
                        ...
                        if (this.getView().byId("fbinp-ticketNumber").getTokens().length > 0) {
                            ...
                            filterval.PRIMARYDOCUMENTNBR = ticarr;
                        ...
                        if (this.getView().byId("fbdat-DateofIssue").getValue()) {
                            if (this.byId("fbdat-DateofIssue").isValidValue()) {
                                ...
                                filterval.issuanceFrom = formattedDates[0].trim();
                                filterval.issuanceTo = formattedDates[1].trim();
                            } else {
            ...
            onScroll: function (oEvent) {
                ...
                var totalPages = Math.ceil(this.GSTDetails.totalcount / 5);
                ...
                if (this.loopfilters.pageNumber <= totalPages) {
                    ...
                    this.SendRequest(this, "/portal-api/portal/v1/get-exhaustive-report", "POST", {}, JSON.stringify(this.loopfilters), (_self, data, message) => {
                        if (data.documentHistory.length > 0) {
                            _self.GSTDetails.documentHistory = [..._self.GSTDetails.documentHistory, ...data.documentHistory];
            ...
            handleValueHelpSearchinvoiceNum: function (oEvent) {
                ...
                var invnoSH = oEvent.getParameter("value");
                if (invnoSH != "") {
                    this.SendRequest(this, "/portal-api/portal/v1/get-filter-data", "POST", {}, JSON.stringify({ "invoiceNumber": invnoSH, "apiType": "DocumentHistoryPriorTo" }), (_self, data, message) => {
            ...
            handleValueHelpSearchpnrNum: function (oEvent) {
                ...
                var invnoSH = oEvent.getParameter("value");
                if (invnoSH != "") {
                    this.SendRequest(this, "/portal-api/portal/v1/get-filter-data", "POST", {}, JSON.stringify({ "pnr": invnoSH, "apiType": "DocumentHistoryPriorTo" }), (_self, data, message) => {
            ...
            handleValueHelpiataNum: function (oEvent) {
                ...
                this.InputId = oEvent.getSource().getId().split("--")[1];
                if (!this.iataNumberDialog) {
                    this.iataNumberDialog = sap.ui.xmlfragment("airindiagst.view.Fragment.iata", this);
            ...
            handleValueHelpSearchticketNum: function (oEvent) {
                ...
                var invnoSH = oEvent.getParameter("value");
                if (invnoSH != "") {
                    this.SendRequest(this, "/portal-api/portal/v1/get-filter-data", "POST", {}, JSON.stringify({ "ticketNumber": invnoSH, "apiType": "DocumentHistoryPriorTo" }), (_self, data, message) => {
            ...
            onButtonDownloadPress: function (oEvent) {
                ...
                const oTableData = this.getView().getModel("GSTDetailsModel").getData().documentHistory;
                        ...
                        if (oSelIndices.length > 100) {
                            ...
                            sap.m.MessageBox.warning('You have selected more than 100 invoices for downloading.It will take some time to download.\n Do you want to continue ?', {
                                ...
                                onClose: function (oAction) {
                                    if (oAction === "Yes") {
                                        sap.ui.core.BusyIndicator.show();
                                        this.SendRequest(this, "/portal-api/portal/v1/archived-invoice-download", "POST", {}, JSON.stringify({ invoices: aInvoices }), (_self, data, message) => {
            ...
            onButtonDownloadPresssingle: function (oEvent) {
                ...
                //     });
                this.SendRequest(this, "/portal-api/portal/v1/archived-invoice-download", "POST", {}, JSON.stringify({ invoices: aInvoices }), (_self, data, message) => {
                    const invoices = data.documentHistory;
                    if (invoices.length > 0) {
            ...
            handleValueHelpSearchbuyername: function (oEvent) {
                ...
                var oFilter = new sap.ui.model.Filter({
                    filters: [
                        new sap.ui.model.Filter("billToName", sap.ui.model.FilterOperator.Contains, sValue),
                    ],
                    and: false
                });
                oEvent.getSource().getBinding("items").filter([oFilter]);
            ...
            handleValueHelpSearchpassengername: function (oEvent) {
                ...
                var oFilter = new sap.ui.model.Filter({
                    filters: [
                        new sap.ui.model.Filter("passengerName", sap.ui.model.FilterOperator.Contains, sValue),
                    ],
                    and: false
                });
                oEvent.getSource().getBinding("items").filter([oFilter]);
            ...
            _registerForP13n: function () {
                ...
                this.oMetadataHelper = new MetadataHelper([
                    ...
                    { key: "col_documentTypeCode", label: "Document Type", path: "DOCUMENTTYPECODE" },
                    { key: "col_invoiceNumber", label: "Invoice Number", path: "INVOICENUMBER" },
                    { key: "col_invoiceDate", label: "Invoice Date", path: "INVOICEDATE" },
                    { key: "col_ticketNumber", label: "Ticket Number", path: "TICKETNUMBER" },
                    { key: "col_ticketdate", label: "Ticket Issue Date", path: "INVOICEDATE" },
                    { key: "col_billToName", label: "Buyer Name", path: "PASSANGERNAME" },
                    { key: "col_iataNumber", label: "IATA Number", path: "IATANUMBER" },
                    { key: "col_passangerName", label: "Passanger Name", path: "PASSANGERNAME" },
                    { key: "col_passengerGSTIN", label: "Passenger GSTIN", path: "PASSENGERGSTIN" },
                    { key: "col_netTaxableValue", label: "Net Taxable Value", path: "NETTAXABLEVALUE" },
                    { key: "col_cominedtax", label: "Combined Tax Rate", path: "TOTALRATE" },
                    { key: "col_totaltax", label: "Total Tax", path: "NETTAXABLEVALUE" },
                    { key: "col_cgstRate", label: "CGST Rate", path: "CGSTRATE" },
                    { key: "col_cgstamount", label: "CGST Amount", path: "CGSTAMOUNT" },
                    { key: "col_sgstRate", label: "SGST Rate", path: "SGSTRATE" },
                    { key: "col_sgstamount", label: "SGST Amount", path: "SGSTAMOUNT" },
                    { key: "col_utgstRate", label: "UTGST Rate", path: "UTGSTRATE" },
                    { key: "col_utgstamount", label: "UTGST Amount", path: "UTGSTAMOUNT" },
                    { key: "col_igstRate", label: "IGST Rate", path: "IGSTRATE" },
                    { key: "col_igstamount", label: "IGST Amount", path: "IGSTAMOUNT" }
                ]);
