...
exports.getUserDetails = async (db, email) => {
    try {
      ...
      const query = `SELECT COMPANYUSERS.COMPANYID, COMPANYUSERS.LOGINEMAIL, COMPANYUSERS.FIRSTNAME, COMPANYUSERS.LASTNAME, COMPANYUSERS.MOBILE FROM COMPANYUSERS WHERE LOGINEMAIL = ? AND STATUS != ?`;
...
exports.getCountryFromCompany = async (db, countryCode) => {
  try {
    ...
    const query = `SELECT * FROM COMPANYMASTER WHERE CONSULATEEMBASSYCOUNTRY_CODE = ?`;
...
exports.getIATAFromCompany = async (db, iataCode) => {
  try {
    ...
    const query = `SELECT * FROM COMPANYMASTER WHERE AGENTCODE = ?`;
...
exports.getGSTINFromCompany = async (db, param1) => {
  try {
    ...
    const query = `SELECT * FROM COMPANYMASTER WHERE UNSHORTCODE = ?`;
...
exports.getCompanyGstinName = async (db, gstin) => {
  try {
    if (!db) throw new Error('DB Connection is required');
    ...
    const query = `SELECT TOP 1
    LEGALNAMEOFBUSINESS
    FROM UNBODYMASTER where GSTINUIN =  ?`;
    const companyUsers = (await db.exec(query,[gstin]))[0]
...
/**
  ...
  * @returns {Object} - JSON Object with fields status, message and data (Array of User Details).
*/
exports.getUserData = async (tx, email) => {
  try {
    ...
    const companyUsers = (await tx.run(query,[email]))[0]
