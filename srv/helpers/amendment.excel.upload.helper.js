 ...
 */
exports.processAmendmentRequestsExcel = async (tx, base64Excel, FirstName, Email) => {
        ...
        if (!isCellsMissing) {
            ...
            for (const entry of changeGSTINSheetEntries) {
                ...
                try {
                    ...
                    const [originalInvoice] = await tx.run(originalInvoiceQuery, [invoiceNumber]);
                    ...
                    const result = await makeGSTINAmendment(tx, invoiceNumber, newGSTIN, isAmended, status, Email, timestamp, 'anonymous', originalInvoice, FirstName, amendmentReason);
        ...
        if (!isCellsMissing) {
            ...
            for (const entry of removeGSTINSheetEntries) {
                ...
                try {
                    ...
                    const status = 'AF';
                    const timestamp = new Date().toISOString().replace('T', ' ').split('.')[0];
                    const result = await makeAddressAmendment(tx, address, invoiceNumber, isAmended, status, Email, timestamp,'anonymous', originalInvoice, FirstName, amendmentReason);
                    processedEntry.remarks = result.message;
                    processedRemoveGSTINSheetEntries.push(processedEntry);
        ...
        if (!isCellsMissing) {
            ...
            for (const entry of changeAddressSheetEntries) {
                ...
                try {
                    ...
                    const status = 'AF';
                    const timestamp = new Date().toISOString().replace('T', ' ').split('.')[0];
                    const result = await makeChangeAddressAmendment(tx, address, invoiceNumber, isAmended, status, Email, timestamp,'anonymous',  originalInvoice, FirstName, amendmentReason);
                    processedEntry.remarks = result.message;
                    processedChangeAddressSheetEntries.push(processedEntry);
