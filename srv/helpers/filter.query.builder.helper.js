...
exports.buildQuery = (baseQuery, input, pageNumber,pageSize,) => {
    let query = '';
    let conditions = [];
            ...
            switch (key) {
                ...
                case 'invoiceStatus':
                    if(input[key] == 'non-cancelled'){
                        conditions.push(`(INVOICESTATUS <> 'N' OR INVOICESTATUS IS NULL)`);
                    }
                    if(input[key] == 'cancelled'){
                        conditions.push(` INVOICESTATUS = 'N'`);
                    }
                break;
                ...
                case 'status':
                    let status = ""
                    if(input["status"] == "approved") status = "A"
                    else if(input["status"] == "rejected") status = "R";
