...
const { getAdminsFromCompanyId } = require('../db/agent.db');
...
const sendAmendmentRequestMailToAdmin = async (tx, companyId, newValue, oldValue, type) => {
    ...
    try {
        ...
        const admins = (await getAdminsFromCompanyId(tx, companyId)).data
        // console.log(admins);
        ...
        const mailOptions = {
            from: 'noreply@airindia.com',
            to: email,
            subject: type+' Amendment Request',
            html: updatedHTML
        };
...
const sendAmendmentApproveorRejectMailToUser = async (name, email, newValue, oldValue, type, mailType) => {
    ...
    try {
        ...
        const mailOptions = {
            from: 'noreply@airindia.com',
            to: email,
            subject: `${type} Amendment Request ${mailType}`,
            html: updatedHTML
        };
...
const sendAmendmentRequestMailToAIAdmins = async (tx, newValue, oldValue, type) => {
    ...
    try {
        ...
        const admins = (await getAISuperAdmins(tx)).data
        // console.log(admins);
